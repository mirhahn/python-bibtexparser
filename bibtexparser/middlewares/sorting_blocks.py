...
def fallback_key(
    ...
    '''
    Make key generator exception-proof.

    Some key generators only work for some types of blocks. Creating a
    `type_key` can be cumbersome if your key generator applies to many
    different types of blocks. In this case, the `fallback_key` substitutes
    a fallback key value for those blocks where the inner key generator
    fails.

    Args:
        key: Description of the inner key generator. When possible, the key
            generated by the inner key generator is used.
        fallback: Key value to use when the inner key generator raises an
            expected exception.
        exc_cls: Enumeration of expected exception types. Defaults to all
            sub-types of `Exception`. It is recommended to be as specific as
            possible to avoid unintentionally masking types like `MemoryError`
            and `KeyboardInterrupt`.

    Returns:
        A key generator function that returns the same key as the inner key
        generator, except in cases where the inner key generator raises an
        exception listed in `exc_cls`. In those cases, it returns `fallback`.
        Other exceptions are still passed on to the caller.
    '''
